...
Evaluation metrics implemented from scratch (no sklearn.metrics):
 - confusion matrix
 - overall accuracy
 - per class precision, recall, f1
 - macro average and micro average precision/recall/f1
 - RMSE and %RMSE (for the regression tasks)
...
def per_class_precision_recall_f1(cm, labels):
    ...
    for i, lab in enumerate(labels):
        tp = cm[i, i]
        fp = np.sum(cm[:, i]) - tp   # predicted as lab, but wasn't
        fn = np.sum(cm[i, :]) - tp   # actually lab, but predicted as something else
...
def micro_average_precision_recall_f1(cm):
    # micro averaging pools all TP/FP/FN across classes first
    n = cm.shape[0]
    tp_total = 0
    fp_total = 0
    fn_total = 0
...
def print_all_metrics(y_true, y_pred, labels=None):
    # one function that computes and prints everything: confusion matrix,
    # accuracy, per class precision/recall/f1, macro avg, micro avg
