...
Helper functions for:
 - splitting each class into train/test (70/30)
 - generating one-against-one class pairs for multiclass classification
 - combining predictions from multiple pairwise classifiers by majority vote
...
def train_test_split_per_class(X, y, train_frac=0.7, random_state=None):
    # split 70/30 separately within each class, so each class is
    # represented in the same proportion in train and test
...
def subset_for_pair(X, y, class_a, class_b):
    # keep only the rows belonging to class_a or class_b
    mask = (y == class_a) | (y == class_b)
    return X[mask], y[mask]


def aggregate_votes(vote_matrix, classes):
    # vote_matrix: (n_samples, n_pairs), each column is the class label
    # predicted by one pairwise classifier. Take the majority vote for
    # each sample.
